...
def login_user():
    try:
        ...
        # Generate a JWT token with automatic expiration (e.g., 1 hour)
        ...
        employee_id_value = str(
            user.employee_id) if user.employee_id is not None else ""

        current_time = datetime.now()
        expiration_time = current_time + timedelta(hours=1)
        hash_key = token
        redis_client.hset(hash_key, "token", token)
        redis_client.hset(hash_key, "user_id", user.id)
        redis_client.hset(hash_key, "employee_id", employee_id_value)
        redis_client.hset(hash_key, "first_name", user.first_name)
        redis_client.hset(hash_key, "last_name", user.last_name)
        redis_client.hset(hash_key, "pfs_num", user.pfs_num)
