...
import {
	changeLimit,
	confirmAction,
	formatDate, formatFullName, formatGetInitialsName, getHashString, getQueryString,
...
import {
	DELETE_EMPLOYEE_API,
	DELETE_EMPLOYEE_AWARD_API, FETCH_EMPLOYEE_POSTINGS_API,
	FETCH_EMPLOYEES_API,
...
const RetiredEmployees = () => {
	...
	const fetchProfiles = useCallback(async (per_page, page, q, filters='') => {
		try {
			const rs = await request(
				`${FILTER_EMPLOYEES_API}?per_page=${per_page}&page=${page}&employment_status=1&q=${q}${filters}`
			);
	...
	useEffect(() => {
		...
		if (
			...
			if (_page !== page || _search !== search || _limit !== queryLimit || _filterQuery !== filterQuery) {
				setFetching(true);
			}
	...
	return (
		<>
			<div className="container-fluid">
				<Breadcrumbs pageTitle="Retired employees" parentPage="Employees" />
				<div className="row">
											...
											{list.map((item, i) => {
												...
												return (
													<tr key={i}>
															...
															<div className="d-flex align-items-center">
																							<div className="flex-shrink-0 me-2">
																								<div className="flex-shrink-0 avatar-sm">
																									{item.photo?
																										<img src={item.photo} alt="" className="avatar-sm p-0 rounded-circle"/>:
																							...
																							<div>
																								<a href={`/employees/${item.id}/view`} 	className="text-reset text-underline">
														...
														<td>{formatDate(item.date_of_retirement)}</td>
