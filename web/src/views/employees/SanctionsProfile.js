...
import {
	...
	DELETE_EMPLOYEE_POSTINGS_API, DELETE_EMPLOYEE_SANCTION_API, DELETE_EMPLOYEE_TRAINING_API,
	FETCH_EMPLOYEE_DEPLOYMENT_BY_EMPLOYEE_API,
	FETCH_EMPLOYEE_POSTINGS_API,
	FETCH_EMPLOYEE_POSTINGS_BY_EMPLOYEE_API, FETCH_EMPLOYEE_SANCTIONS_BY_EMPLOYEE_API,
...
const TrainingsProfile = () => {
	...
	const fetchPostings = useCallback(async (per_page, page, q,  filters='') => {
		try {
			...
			const rs = await request(FETCH_EMPLOYEE_SANCTIONS_BY_EMPLOYEE_API.replace(':id', params.id));


			const { emp_sanctions, ...rest } = rs;

			setList(emp_sanctions);
	...
	return (
		...
		<div className="container-fluid">
			<Breadcrumbs pageTitle="Employee Sanctions" parentPage="Accounts" />
			<div className="row">
				<div className="col-lg-12">
					<div className="card">
						<TitleSearchBar
							title={`Sanction History - ${formatFullName(employeeData)}`}
							onClick={() => addPosting()}
							...
							linkTo={`/employees/sanctions`}
							createBtnTitle="Back to Sanctions"
							...
							>
								...
								{list.length === 0 && (
									<div className="noresult py-5">
										<NoResult title="Employee Sanctions" />
									</div>
								)}
							</TableWrapper>
