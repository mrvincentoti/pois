...
import {
	...
	FETCH_EMPLOYEE_DUE_FOR_RETURN_POSTINGS_API,
	FETCH_EMPLOYEES_API,
	FETCH_STATIONS_API,
	...
	const fetchPostings = useCallback(async (per_page, page, q, filters = '') => {
		try {
			const rs = await request(
				`${FETCH_EMPLOYEE_DUE_FOR_RETURN_POSTINGS_API}?per_page=${per_page}&page=${page}&q=${q}${filters}`
			);

			const { due_for_return_posting, ...rest } = rs;
			setList(due_for_return_posting);
	...
	return (
		<>
			<div className="container-fluid">
				<Breadcrumbs pageTitle="Returning From Post" parentPage="Accounts" />
				<div className="row">
					<div className="col-lg-12">
						<div className="card">
							<TitleSearchBar
								title="Employees Returning in 4 Months"
								// onClick={() => addPosting()}
									...
									<table className="table align-middle table-nowrap">
										<thead className="table-light">
											<tr>
												...
												<th className="text-uppercase">ASM DATE</th>
												<th className="text-uppercase">E.D.R</th>
												<th className="text-uppercase">D.O.R</th>
														...
														<td className="text-end">
															<div className="hstack gap-3 text-end">
																...
																<div className="hstack gap-3 flex-wrap text-end">
																	{item.is_extended === false &&
																	item.is_recall === false &&
																	item.posting_type !== 4 &&
																	item.date_of_return == null ? (
																		<Dropdown>
																			<Dropdown.Toggle
																				variant="light"
																				id="defaultDropdown"
																				className="vertical-ellipsis-toggle"
																			></Dropdown.Toggle>
																			...
																			>
																				<Dropdown.Item
																					...
																					onClick={() => editPosting(item)}
																				>
																					{' '}
																					<i className="ri-edit-2-line" /> Edit
																				</Dropdown.Item>
																				<Dropdown.Item
																					onClick={() => extendPosting(item)}
																					className="text-primary"
																				>
																					<i className="ri-clockwise-2-fill" />{' '}
																					Extend
																				</Dropdown.Item>
																				<Dropdown.Item
																					onClick={() => crossPosting(item)}
...
export default ReturningFromPosting;
