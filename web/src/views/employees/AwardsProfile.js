...
const TrainingsProfile = () => {
	document.title = `Employee Awards History - ${APP_SHORT_NAME}`;
	...
	const fetchPostings = useCallback(async (per_page, page, q,  filters='') => {
		try {
			...
			const rs = await request(FETCH_EMPLOYEE_AWARD_BY_EMPLOYEE_API.replace(':id', params.id));


			const { emp_awards, ...rest } = rs;
	...
	return (
		...
		<div className="container-fluid">
			<Breadcrumbs pageTitle="Employee Awards" parentPage="Accounts" />
			<div className="row">
				<div className="col-lg-12">
					<div className="card">
						<TitleSearchBar
							title={`Awards History - ${formatFullName(employeeData)}`}
							onClick={() => addPosting()}
							...
							linkTo={`/employees/awards`}
							createBtnTitle="Back to Awards"
