...
import {
	...
	DELETE_EMPLOYEE_POSTINGS_API, FETCH_EMPLOYEE_DEPLOYMENT_BY_EMPLOYEE_API,
	FETCH_EMPLOYEE_POSTINGS_API, FETCH_EMPLOYEE_POSTINGS_BY_EMPLOYEE_API, FETCH_EMPLOYEES_API,
	FETCH_STATIONS_API,
	FILTER_EMPLOYEE_POSTINGS,
	FILTER_EMPLOYEE_POSTINGS_API,
	FILTER_EMPLOYEES_API, GET_EMPLOYEE_API,
} from '../../services/api';
...
const DeploymentProfile = () => {
	...
	const fetchPostings = useCallback(async (per_page, page, q,  filters='') => {
		try {
			...
			const rs = await request(FETCH_EMPLOYEE_DEPLOYMENT_BY_EMPLOYEE_API.replace(':id', params.id));


			const { employee_deployments, ...rest } = rs;

			setList(employee_deployments);
	...
	return (
		...
		<div className="container-fluid">
			<Breadcrumbs pageTitle="Deployments" parentPage="Accounts" />
			<div className="row">
				<div className="col-lg-12">
					<div className="card">
						<TitleSearchBar
							title={`Deployment History - ${formatFullName(employeeData)}`}
							onClick={() => addPosting()}
							...
							linkTo={`/employees/deployments`}
							createBtnTitle="Back to Deployments"
