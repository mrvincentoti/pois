...
import { DELETE_EMPLOYEE_API, FETCH_EMPLOYEES_API } from '../../services/api';
	...
	const fetchProfiles = useCallback(async (per_page, page, q, filters = '') => {
		try {
			const rs = await request(
				`${FETCH_EMPLOYEES_API}?per_page=${per_page}&page=${page}&q=${q}${filters}`
			);
	...
	return (
		<>
			<div className="container-fluid">
				<Breadcrumbs pageTitle="Profiles" parentPage="Employees" />
				<div className="row">
											...
											{list.map((item, i) => {
												return (
													<tr key={i}>
														...
														<td>{item.cadre?.name || 'N/A'}</td>
														<td>{item.directorate?.name || 'N/A'}</td>
