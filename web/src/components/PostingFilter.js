...
const PostingFilter = ({ show, onCloseClick, onFilter, onClearFilter }) => {
	...
	const fetchRegion = useCallback(async () => {
		try {
			const rs = await request(`${FETCH_REGIONS_API}?page=1&per_page=10`);
	...
	const setFilters = useCallback(() => {
		...
		if (filters) {
			...
			setSelectedStation(
				filters.station_id
					? stations.find(station => station.id === filters.station_id)
					: null
			);
	...
	}, [location.hash, regions, stations]);
	...
	useEffect(() => {
		...
		clearFilterParams();
	}, [clearFilterParams, loaded, setFilters, fetchRegion, fetchStations]);
	...
	const doFilter = e => {
		...
		const filters = {
			...
			...(dorStartDate
				? { expected_date_of_return_start_date: dorStartDate }
				: ''),
